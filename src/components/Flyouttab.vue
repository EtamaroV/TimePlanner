...
<template>
    ...
    <div class="backdrop-flyouttab">
        ...
        <div class="main-flyouttab">
            <button class="backbtn-flyouttab" @click="goBack" v-if="router.currentRoute.value.path.split('/').length > 2 && (router.currentRoute.value.path.split('/').slice(-1)[0] !== '') && (JSON.stringify(poplast(router.currentRoute.value.path)) !== JSON.stringify(close_route.split('/')))">
